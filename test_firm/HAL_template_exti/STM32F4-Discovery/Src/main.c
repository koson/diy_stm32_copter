...
uint8_t count=0;
uint32_t servo_read_start, servo_read_stop, servo_read, count_servo;
...
//	}
	if (GPIO_Pin == GPIO_PIN_5){
		if (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_5) == 1){
			count_servo++;
			servo_read_start = TIM5->CNT;
		}
		else {
			servo_read_stop = TIM5->CNT;
			servo_read = servo_read_stop - servo_read_start;
